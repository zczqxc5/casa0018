/*
 This is a model trained by edge impluse to recognize fitness movements, designed to deploy on the Arduino nano.  
 Its function is mainly to identify the fitness action and the corresponding indicator light flashes. 
 When the counter of an action reaches 10, its corresponding indicator light is keeping on to indicate the completion of fitness.

 Connection: The long feet of the LED are connected to D2,D3,D4,D5,D6, and the short feet to grounded
 ...
 * Reference: https://docs.edgeimpulse.com/docs/tutorials/end-to-end-tutorials/continuous-motion-recognition
 */

/* Includes ---------------------------------------------------------------- */
#include <warm_inferencing.h>
...
int actionCounts[5] = { 0, 0, 0, 0, 0 };
const char *actionNames[5] = { "Dumbbell Row", "Hammer Curls", "other", "Upright Row", "deep squat" };
...
void loop() {
  ...
  if (maxValue > 0.5 && maxIndex != -1) {
    actionCounts[maxIndex]++;


    // count print
    ei_printf("\count:\r\n");
    for (size_t ix = 0; ix < 5; ix++) {
      ei_printf("%s: %d\r\n", actionNames[ix], actionCounts[ix]);
    }
  }
  ...
  if (maxValue > 0.5 && maxIndex != -1) {
    ...
    if (maxIndex != 2 && actionCounts[maxIndex] >= 10) { 
        digitalWrite(ledPins[maxIndex], HIGH); 
    } else if (maxIndex == 2) { 
      
        int blinkCount = 3;
        for (int j = 0; j < blinkCount; j++) {
            digitalWrite(ledPins[maxIndex], HIGH); // LED ON
            delay(300);
            digitalWrite(ledPins[maxIndex], LOW); // LED OFF
            if (j < blinkCount - 1) {
                delay(300);
            }
        }
